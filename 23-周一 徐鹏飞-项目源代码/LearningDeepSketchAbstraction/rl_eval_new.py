...
# todo: reflect ss_idx to plotting
...
def plot_action_prob():
    msg: str = ""
    remove_probs = []
    keep_probs = []

    with open("./action_prob_record", "r") as f:
        while True:
            msg = f.readline()
            if msg == "":
                break
            probs_str = msg.split(" ")
            remove_probs.append(float(probs_str[0]))
            keep_probs.append(float(probs_str[1]))

    plt.plot(range(len(remove_probs)), remove_probs, 'r')
    plt.plot(range(len(remove_probs)), keep_probs, 'b')
    plt.xlabel("train step")
    plt.ylabel("prob")
    plt.legend("remove", "keep")
    plt.show()
    return
